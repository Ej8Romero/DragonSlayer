...
def battle():
    global player_hp, spider_hp, player_attack, player_defense  # Keep track of HP and stats throughout the battle
    ...
    while player_hp > 0 and spider_hp > 0:
        ...
        if choice == "attack":
            # Player attacks: roll a d6 and multiply by player attack
            ...
            # Player defends: roll a d4 and multiply by player defense
            defense_block = random.randint(1, 4) * player_defense
            damage_taken = max(0, spider_attack - defense_block)  # Ensure no negative damage
        ...
        # Monster's turn (the monster always attacks)
        if spider_hp > 0:
            damage_taken = random.randint(1, 6) * spider_attack  # Roll for monster attack
            player_hp -= damage_taken
...
# Reward function for winning the battle
def reward_choice():
